...
class MarkovGaussianProcess(BaseModel):
    ...
    def likelihood_cov(self, X=None, R=None, pseudo_lik_params=None, cubature=None):
        """
        Predicts at new test locations X and returns the full likelihood covariance matrix.
        """
        if X is None:
            X = self.X
        elif len(X.shape) < 2:
        ...
        var_f = diag(test_var)
        mean_f = test_mean.reshape(test_mean.shape[0], -1, 1)
        if not isinstance(self.likelihood, MultiLatentLikelihood):
            var_f = var_f.reshape(var_f.shape[0], -1, 1)
        mean_y, var_y = vmap(self.likelihood.predict, (0, 0, None))(mean_f, var_f, cubature)

        var_y = np.squeeze(var_y)
        if not var_y.shape:
            output_var = np.array([var_y])
        else:
            diag_var = np.diag(var_y)
            output_var = np.squeeze(test_var)
            n = output_var.shape[0]
            output_var = (np.ones((n,n)) - np.diag(np.ones(n)))*np.squeeze(test_var) + diag_var

        return np.squeeze(test_mean), output_var
